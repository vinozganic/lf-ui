...
const MatchCard = ({ match, lostItem, handleShowProps, handleDiscard }) => {
    ...
    const trashCanSVG = (
        <svg className={`svg-icon h-8 max-sm:h-6`} viewBox="0 0 20 20">
		    <path fill="white" d="M16.471,5.962c-0.365-0.066-0.709,0.176-0.774,0.538l-1.843,10.217H6.096L4.255,6.5c-0.066-0.362-0.42-0.603-0.775-0.538C3.117,6.027,2.876,6.375,2.942,6.737l1.94,10.765c0.058,0.318,0.334,0.549,0.657,0.549h8.872c0.323,0,0.6-0.23,0.656-0.549l1.941-10.765C17.074,6.375,16.833,6.027,16.471,5.962z"></path>
			<path fill={`${match.discarded ? "none" : "white"}`} d="M16.594,3.804H3.406c-0.369,0-0.667,0.298-0.667,0.667s0.299,0.667,0.667,0.667h13.188c0.369,0,0.667-0.298,0.667-0.667S16.963,3.804,16.594,3.804z"></path>
			<path fill={`${match.discarded ? "none" : "white"}`} d="M9.25,3.284h1.501c0.368,0,0.667-0.298,0.667-0.667c0-0.369-0.299-0.667-0.667-0.667H9.25c-0.369,0-0.667,0.298-0.667,0.667C8.583,2.985,8.882,3.284,9.25,3.284z"></path>
		</svg>
    )

    const infoSVG = (
        <svg className={`svg-icon h-8 max-sm:h-6`} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path fill="white" d="M11 10.9794C11 10.4271 11.4477 9.97937 12 9.97937C12.5523 9.97937 13 10.4271 13 10.9794V16.9794C13 17.5317 12.5523 17.9794 12 17.9794C11.4477 17.9794 11 17.5317 11 16.9794V10.9794Z"></path>
            <path fill="white" d="M12 6.05115C11.4477 6.05115 11 6.49886 11 7.05115C11 7.60343 11.4477 8.05115 12 8.05115C12.5523 8.05115 13 7.60343 13 7.05115C13 6.49886 12.5523 6.05115 12 6.05115Z"></path>
            <path fillRule="evenodd" fill="white" d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2ZM4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12Z"></path>
        </svg>
    )

    useEffect(() => {
        if (match.data.match_probability * 100 > 80)
            setProgressColor("bg-green")
        else if (match.data.match_probability * 100 > 50)
            setProgressColor("bg-yellow")
        else 
            return setProgressColor("bg-red")
    })

    const CardButton = ({iconSVG, onClickHandler, className}) => {
        return (
            <div className={`flex items-center justify-center text-center border-2
                font-bold p-3 rounded-full lg:hover:bg-opacity-50 cursor-pointer w-fit
                transition-all ease-in-out duration-150 ${className}`}
                onClick={(e) => { e.stopPropagation(); onClickHandler(match.data.id) }}>
                {iconSVG}
            </div>
        )
    }
    ...
    const ProgressBar = () => {
        return (
            <div className="w-full items-center justify-start flex h-8 p-1 rounded-2xl border-2 border-white/50 bg-white/20 border-opacity-50">
                <div className={`h-full rounded-2xl ${progressColor} px-4 flex justify-center items-center text-background font-bold`} 
                    style={{ width: `${match.data.match_probability * 100}%` }}>
                    {match.data.match_probability*100}%
                </div>
            </div>
        )
    }
