...
const MatchCard = ({ probability, matchId, lostItem, functionPassID, showProps, handleShowProps, discarded, handleDiscard }) => {
    ...
    return (
        <div className="lg:scale-100 md:scale-100 max-sm:scale-90">
            <div ref={ref} className={`relative py-6 pl-6 md:pl-8 lg:pl-6 max-md:pr-6 md:pr-8 lg:pr-0 items-center justify-center flex select-none bg-gray/60 border-2 
                ...
                onClick={() => {if (!discarded) {handleShowProps(functionPassID)}; handleFlashClick() } }>
                <div className={`w-full grid gap-y-10`}>
                    <MediumText className={`max-lg:text-center ${discarded ? "text-white/40" : "text-white"}`}>{`Vjerojatnost: ${probability}`}</MediumText>
                    <div className="flex lg:justify-start gap-x-5 max-lg:justify-center">
                        <div className={`${(discarded && screenWidth < 1024) ? "hidden" : ""} flex gap-2 text-center font-bold px-6 py-2 w-fit items-center justify-center
                            ...
                            <svg className="svg-icon h-[26px]" viewBox="0 0 20 20">
                        ...
                        <div className={`${(discarded && screenWidth < 1024) ? "hidden" : ""} flex gap-2 text-center font-bold px-6 py-2 w-fit items-center justify-center
                            ...
                            <div className="text-xl font-bold text-black">Discard</div>
                        ...
                        <div className={`${(discarded && screenWidth < 1024) ? "visible" : "hidden"} flex gap-2 text-center font-bold px-6 py-2 w-fit items-center justify-center
                            rounded-3xl hover:bg-opacity-80 bg-primary`}
                            onClick={(e) => { e.stopPropagation(); handleDiscard(functionPassID); }}>
            ...
            {discarded &&
                <div className={`max-lg:hidden absolute z-10 opacity-0 hover:opacity-100 h-[168px] w-[434px] max-md:hidden
                    ...
                    <div className="flex grid-flow-row justify-center items-center gap-x-2 w-fit h-fit cursor-pointer bg-primary/60 border-2 border-black rounded-3xl px-6 py-2 hover:bg-primary/50"
